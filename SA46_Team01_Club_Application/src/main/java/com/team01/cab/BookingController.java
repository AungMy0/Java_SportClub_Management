...
public class BookingController {
				...
				final RedirectAttributes redirectAttributes, @RequestParam("facilityId") int facilityId)
 {
	...
	public ModelAndView bookingfilListPage(HttpServletRequest req ,@ModelAttribute Booking booking, BindingResult result,
			final RedirectAttributes redirectAttributes ,
			@RequestParam("startdate")@DateTimeFormat(pattern = "MM/dd/yyyy") Date sDate ,
			@RequestParam("enddate")@DateTimeFormat(pattern = "MM/dd/yyyy") Date eDate) {
		ModelAndView mav = new ModelAndView("cancelbooking");
	
		ArrayList<Booking> bookingList = bservice.findcancelBooking(sDate, eDate);
		mav.addObject("bookingList",bookingList);
		return mav;
	}
	
	@RequestMapping(value = "/cancellist", method = RequestMethod.GET)
	public ModelAndView bookingCancelSearchPage() {
		ModelAndView mav = new ModelAndView("cancelBooklistSearch", "booking", new Booking());
		return mav;
	...
	{
		ModelAndView mav = new ModelAndView("redirect:/booking/cancellist");
		Booking booking = bservice.findBooking(id);
		bservice.removeBooking(booking);
		String message = "The facility " + booking.getBookingNo() + " was successfully deleted.";
